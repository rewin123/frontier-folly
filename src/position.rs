...
impl SpacePosition {
    pub fn smooth(
        &self,
        new_position: &SpacePosition,
        lag_weight: f32,
        grid_settings: &big_space::FloatingOriginSettings,
    ) -> SpacePosition {
        ...
        let sub_dvec = grid_settings
            .grid_position_double(&sub.cell, &Transform::from_translation(sub.position));
        let sub_dvec = sub_dvec * lag_weight as f64;
        let sub = grid_settings.translation_to_grid(sub_dvec);
        let sub = SpacePosition {
            cell: sub.0,
            position: sub.1,
        };
        *new_position + sub
